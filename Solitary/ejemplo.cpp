...
void mostrarCarta(Pila pila[], int j) {
	if(V[j].estado == 1)
        if(V[j].color == 1) {
            printf("[%s%d%c%s]", ANSI_COLOR_RED, V[j].numeroCarta, V[j].paloCarta, ANSI_COLOR_RED);
        } else {
            printf("[%s%d%c%s]", ANSI_COLOR_RESET, V[j].numeroCarta, V[j].paloCarta, ANSI_COLOR_RESET);
        }
		
	else
        if(V[j].color == 1) {
            printf("[%s%c%c%s]", ANSI_COLOR_RED, '*', '*', ANSI_COLOR_RED);
        } else {
            printf("[%s%c%c%s]", ANSI_COLOR_RESET, '*', '*', ANSI_COLOR_RESET);
        }
